...
export default {
    data() {
        return {
            ...
            emitter: mitt(),
            monitorEventInbox: [],
            monitorEventInboxOpen: false,
            monitorEventInboxLimit: 100,
            monitorEventUnreadCounts: {
                up: 0,
                down: 0,
                other: 0,
            },
    ...
    methods: {
        ...
        initSocketIO(bypass = false) {
            ...
            socket.on("heartbeat", (data) => {
                ...
                // Also add to the monitor event inbox
                if (data.important) {
                    if (this.monitorList[data.monitorID] !== undefined) {
                        this.addMonitorEventMessage(data);
                    }
        ...
        /**
         * Add a monitor status change event to the inbox.
         * @param {object} data Heartbeat payload
         * @returns {void}
         */
        addMonitorEventMessage(data) {
            ...
            const event = {
                id: `${data.monitorID}-${data.time}-${data.status}`,
            ...
            this.monitorEventInbox.unshift(event);

            if (this.monitorEventInbox.length > this.monitorEventInboxLimit) {
                const removedEvents = this.monitorEventInbox.splice(this.monitorEventInboxLimit);

                removedEvents.forEach((removedEvent) => {
                    if (!removedEvent.read && this.monitorEventUnreadCounts[removedEvent.statusType] > 0) {
                        this.monitorEventUnreadCounts[removedEvent.statusType] -= 1;
                    }
                });
            }

            this.monitorEventUnreadCounts[statusType] += 1;
        },

        /**
         * Mark all inbox items as read and reset unread counters.
         * @returns {void}
         */
        markMonitorEventsRead() {
            this.monitorEventInbox.forEach((event) => {
                event.read = true;
            });

            this.monitorEventUnreadCounts = {
                up: 0,
                down: 0,
                other: 0,
            };
        ...
        /**
         * Toggle the monitor event inbox.
         * @param {boolean | null} forceState Force open/close state
         * @returns {void}
         */
        toggleMonitorEventInbox(forceState = null) {
            this.monitorEventInboxOpen = forceState === null ? !this.monitorEventInboxOpen : forceState;
        },

        /**
         * Clear all monitor inbox items.
         * @returns {void}
         */
        clearMonitorEventInbox() {
            this.monitorEventInbox = [];
            this.monitorEventUnreadCounts = {
                up: 0,
                down: 0,
                other: 0,
