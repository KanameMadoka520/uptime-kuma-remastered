    ...
    <div v-if="loadedTheme" class="container mt-3">
        ...
        >
            <div class="sidebar-body">
                ...
                <div class="my-3 form-check form-switch">
                    <input
                        id="enable-default-status-sort"
                        v-model="config.enableDefaultStatusSort"
                        class="form-check-input"
                        type="checkbox"
                    />
                    <label class="form-check-label" for="enable-default-status-sort">
                        {{ $t("statusPageDefaultStatusSort") }}
                    </label>
                    <div class="form-text">
                        {{ $t("statusPageDefaultStatusSortDescription") }}
                    </div>
                </div>

                <div class="my-4">
                    <h5 class="mb-3">玩家展示层配置</h5>
                    <PlanaShowcaseEditor
                        v-if="config.planaShowcaseConfig"
                        :config="config.planaShowcaseConfig"
                        :monitor-options="showcaseMonitorOptions"
                    />
                </div>
        ...
        <!-- Main Status Page -->
        <div :class="{ edit: enableEditMode }" class="main" :style="mainStyle">
            <!-- Logo & Title -->
            <h1 class="mb-4 title-flex">
                ...
                <span v-if="loadedData" class="top-status-pill" :class="overallStatusPillClass">
            ...
            <div class="mb-4">
                ...
                <PlanaStatusShowcase
                    v-if="shouldUsePlanaShowcase"
                    :slug="config.slug || slug"
                    :public-group-list="$root.publicGroupList"
                    :showcase-config="config.planaShowcaseConfig"
                />
                <PublicGroupList
                    v-else
                    ...
                    :default-status-sort-enabled="config.enableDefaultStatusSort"
                />
            </div>
...
import PlanaStatusShowcase from "../components/PlanaStatusShowcase.vue";
import PlanaShowcaseEditor from "../components/PlanaShowcaseEditor.vue";
import { getResBaseURL } from "../util-frontend";
import { createDefaultPlanaShowcaseConfig } from "../config/plana-status-showcase";
import { normalizePlanaShowcaseTheme } from "../config/plana-showcase-themes";
import {
...
export default {
    ...
    computed: {
        ...
        mainStyle() {
            if (!this.enableEditMode || this.$root.isMobile) {
                return {};
            }
        ...
        showcaseMonitorOptions() {
            ...
            return [...monitorMap.values()].sort((a, b) => a.name.localeCompare(b.name));
        },

        shouldUsePlanaShowcase() {
            return !this.editMode && !!(this.config.planaShowcaseConfig && this.config.planaShowcaseConfig.enabled);
        },
    ...
    watch: {
            ...
            if (loggedIn) {
                this.$root.getSocket().emit("getStatusPage", this.slug, (res) => {
                    if (res.ok) {
                        ...
                        if (this.config.enableDefaultStatusSort === undefined) {
                            this.config.enableDefaultStatusSort = true;
                        }

                        this.normalizePlanaShowcaseConfig();
                    } else {
        ...
        selectedMonitor(monitor) {
            if (monitor) {
                ...
                firstGroup.monitorList.push({
                    ...monitor,
                    manualOrder: firstGroup.monitorList.length + 1,
                });
                this.syncManualOrdersFromCurrentList();
    ...
    methods: {
        ...
        loadSidebarWidth() {
            const savedWidth = Number.parseInt(localStorage.getItem("planaStatusSidebarWidth"), 10);
        ...
        clampSidebarWidth(width) {
            const viewportMax = typeof window !== "undefined" ? Math.max(this.sidebarMinWidth, window.innerWidth - 180) : this.sidebarMaxWidth;
            return Math.min(Math.max(width, this.sidebarMinWidth), Math.min(this.sidebarMaxWidth, viewportMax));
        },
        ...
        stopSidebarResize() {
            ...
            document.body.classList.remove("sidebar-resizing");
            localStorage.setItem("planaStatusSidebarWidth", String(this.sidebarWidth));
        },

        normalizePlanaShowcaseConfig() {
            if (!this.config.planaShowcaseConfig) {
                this.config.planaShowcaseConfig = createDefaultPlanaShowcaseConfig();
            }

            const showcaseConfig = this.config.planaShowcaseConfig;
            showcaseConfig.enabled = showcaseConfig.enabled !== false;
            showcaseConfig.showSummaryCards = !!showcaseConfig.showSummaryCards;
            showcaseConfig.showUnassignedMonitors = !!showcaseConfig.showUnassignedMonitors;
            showcaseConfig.featuredRoutes = Array.isArray(showcaseConfig.featuredRoutes) ? showcaseConfig.featuredRoutes : [];
            showcaseConfig.sections = Array.isArray(showcaseConfig.sections) ? showcaseConfig.sections : [];
            showcaseConfig.monitorMetaEntries = Array.isArray(showcaseConfig.monitorMetaEntries)
                ? showcaseConfig.monitorMetaEntries
                : [];

            showcaseConfig.featuredRoutes = showcaseConfig.featuredRoutes.map((route, index) => ({
                id: route.id || `featured-${index}`,
                enabled: route.enabled !== false,
                kicker: route.kicker || "",
                title: route.title || "",
                description: route.description || "",
                theme: normalizePlanaShowcaseTheme(route.theme),
                monitorIds: Array.isArray(route.monitorIds) ? route.monitorIds : [],
                highlightMonitorId: route.highlightMonitorId ?? null,
            }));

            showcaseConfig.sections = showcaseConfig.sections.map((section, index) => ({
                id: section.id || `section-${index}`,
                enabled: section.enabled !== false,
                kicker: section.kicker || "状态分区",
                title: section.title || "",
                description: section.description || "",
                theme: normalizePlanaShowcaseTheme(section.theme),
                monitorIds: Array.isArray(section.monitorIds) ? section.monitorIds : [],
                columns: [1, 2, 3, 4].includes(section.columns) ? section.columns : 3,
            }));

            showcaseConfig.monitorMetaEntries = showcaseConfig.monitorMetaEntries.map((entry, index) => ({
        ...
        /**
         * Ensure every public monitor keeps a stable manual order for edit mode.
         * @returns {void}
         */
        normalizeManualOrders() {
            this.$root.publicGroupList.forEach((group) => {
                group.monitorList.forEach((monitor, index) => {
                    if (typeof monitor.manualOrder !== "number") {
                        monitor.manualOrder = index + 1;
                    }
                });
            });
        },
        ...
        /**
         * Restore all groups to the order configured in the editor.
         * @returns {void}
         */
        restoreManualGroupOrder() {
            this.normalizeManualOrders();

            this.$root.publicGroupList.forEach((group) => {
                group.monitorList = [...group.monitorList].sort((a, b) => {
                ...
                group.sortKey = null;
                group.sortDirection = null;
            });
        },

        /**
         * Sync manualOrder metadata with the current editor order before saving.
         * @returns {void}
         */
        syncManualOrdersFromCurrentList() {
            this.$root.publicGroupList.forEach((group) => {
                group.monitorList.forEach((monitor, index) => {
                    monitor.manualOrder = index + 1;
                });
...
.top-status-pill.is-up {
    color: #15803d;
    background: rgba(34, 197, 94, 0.12);
}

.top-status-pill.is-partial {
    color: #b45309;
    background: rgba(245, 158, 11, 0.14);
}

.top-status-pill.is-down {
    color: #b91c1c;
    background: rgba(239, 68, 68, 0.12);
}

.top-status-pill.is-maintenance {
    color: $maintenance;
    background: rgba(168, 85, 247, 0.12);
}

.top-status-pill.is-unknown {
    color: #475569;
